    ...
    try {
      if (FirebaseApp.getApps(context).isEmpty()) {
        val options = FirebaseOptions.Builder()
          .setApplicationId("1:141923968495:android:com.aistudio.chatmessenger")
          .setProjectId("ais-dev-gnw3dcd26u3w3sal5isrvy")
          .setApiKey("AIzaSyFakeKeyForFirestoreClientInit123")
          .build()
        FirebaseApp.initializeApp(context, options)
      }
      FirebaseFirestore.getInstance()
    } catch (e: Exception) {
      Log.w(TAG, "Firebase initialization notice (offline mode available): ${e.message}")
      null
    }
  }
